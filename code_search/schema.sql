...
CREATE TABLE IF NOT EXISTS code_embeddings_e5 (
    id SERIAL PRIMARY KEY,
    ...
    -- Embedding (1024 dimensions for e5-large-v2)
...
-- HNSW index for e5 embeddings
CREATE INDEX IF NOT EXISTS code_embeddings_e5_vector_hnsw_idx
ON code_embeddings_e5
USING hnsw (embedding vector_cosine_ops)
WITH (m = 16, ef_construction = 64);

CREATE INDEX IF NOT EXISTS code_embeddings_e5_repo_idx
ON code_embeddings_e5(repo_name);

CREATE INDEX IF NOT EXISTS code_embeddings_e5_language_idx
ON code_embeddings_e5(language);

-- Example search query:
-- SELECT file_path, name, content, 1 - (embedding <=> '[0.1, 0.2, ...]'::vector) AS similarity
-- FROM code_embeddings  -- or code_embeddings_e5 for e5 model
